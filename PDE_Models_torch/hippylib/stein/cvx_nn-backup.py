...
def cvx_nn_relu_find_reg(X, sample_num = 200, target_num=0,seed=0):
    ...
    for j in range(m):
        ...
        ABj = np.sum(dj)*I_aug-Y_aug.T@XDj-XDj.T@Y_aug
...
def cvx_nn_relu_dual(X, V, lbd, sample_num = 200 , target_num =0, seed=0, cvx_solver='SCS', cvx_verbose=False, cvx_opts={}):
    N, d = X.shape
    ...
    p_star = cp.Problem(cp.Maximize(-0.5*cp.sum_squares(Y_aug+V_aug)),constraints).solve(solver=solver, 
             verbose=cvx_verbose, **cvx_opts)
...
def cvx_nn_relu_primal(X, V, lbd, sample_num = 200 ,seed=0):
    ...
    for j in range(m):
        ...
        objective = objective+np.sum(dj)*cp.trace((S_m_j-S_p_j)@B0)+tlbd/2*cp.trace((S_m_j+S_p_j)@e@e.T)
