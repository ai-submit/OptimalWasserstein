...
class Kernel:
    ...
    def compute_trace(self):
        if self.type_metric=='prior':
            if self.options["is_projection"]:
                return self.particle.dimension
            else:
                d = self.particle.dimension
                trace = 0
                for i in range(d):
                    phelp = self.particle.generate_vector() 
                    phelp2 = self.particle.generate_vector() 
                    phelp2.zero()
                    dumb_vec = np.zeros(d)
                    dumb_vec[i] = 1
                    phelp.set_local(dumb_vec)
                    self.model.prior.R.mult(phelp, phelp2)
                    trace += phelp2.get_local()[i]
                return trace
        if self.type_metric=='posterior_average':
            if self.options["is_projection"]:
                d = self.particle.dimension
                trace = 0
                for i in range(d):
                    ...
                    dumb_vec[i] = 1
                    phelp.set_local(dumb_vec)
                    self.variation.hessian_average(phelp, phelp2)
                    trace += phelp2.get_local()[i]
                return trace
            else:
                d = self.particle.dimension
                ...
                for i in range(d):
                    ...
                    phelp3 = self.particle.generate_vector() 
                    phelp3.zero()
                    self.variation.hessian_average(phelp2, phelp3)
                    self.model.prior.R.mult(phelp3,phelp2)
                    trace += phelp2.get_local()[i]
                return trace

    def value_kernel(self, n=0, m=0):
        if self.options["is_projection"]:
            pn = self.particle.coefficients[n]  # n = 1, ..., particle.number_particles
            pm = self.particle.coefficients[m]  # m = 1, ..., particle.number_particles_all
        else:
            pn = self.particle.particles[n]  # n = 1, ..., particle.number_particles
        ...
        value = np.exp(-pout.inner(phelp)/(2*self.scale[n]))  # k(pn,pm) = exp(-(pn-pm)^T * M * (pn-pm))

        return value

    def gradient_kernel(self, n=0, m=0):
        # evaluate the gradient of kernel at (pn, pm) with respect to pm, grad k(pn, pm) = 2 * k(pn, pm) * M * (pn - pm)
        if self.options["is_projection"]:
            pn = self.particle.coefficients[n]  # n = 1, ..., particle.number_particles
            pm = self.particle.coefficients[m]  # m = 1, ..., particle.number_particles_all
        ...
        self.metric.mult(phelp, ghelp, n=n)
        value = self.value_kernel(n=n, m=m)
        gradient.axpy(value/self.scale[n], ghelp)

        return gradient.get_local()

    def laplace_kernel(self, n=0, m=0):
        if self.trace==None:
            self.trace = self.compute_trace()
        ...
        phelp.axpy(-1.0, pm)
        self.metric.mult(phelp, ghelp, n=n)
        value = np.exp(-phelp.inner(ghelp)/(2*self.scale[n])) 

        return (ghelp.inner(ghelp)/self.scale[n]**2-self.trace/self.scale[n])*value


    def compute_kernel_mat(self,dtype=0):
        N = self.particle.number_particles_all
        d = self.particle.dimension
        if dtype==0:
            K = np.zeros([N,N])
            for m1 in range(N):
                for m2 in range(m1+1):
                    v = self.value_kernel(m1,m2)
                    if m1==m2:
                        K[m1,m2] = v
                    else:
                        K[m1,m2] = v
        ...
        if dtype==100:
            ...
            for m1 in range(N):
                for m2 in range(m1):
                    grad = self.gradient_kernel(m1,m2)
                    K[m1*d:(m1+1)*d,m2] = grad
                    K[m2*d:(m2+1)*d,m1] = -grad
        if dtype==400:
            K = np.zeros([N,N])
            for m1 in range(N):
                for m2 in range(m1+1):
                    v = self.laplace_kernel(m1,m2)
                    if m1==m2:
                        K[m1,m2] = v
                    else:
                        K[m1,m2] = v
