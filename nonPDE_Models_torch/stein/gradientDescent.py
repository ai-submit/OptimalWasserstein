...
class GradientDescent:
    ...
    def __init__(self, model, particle, variation, kernel, options, comm):
        ...
        if self.WGF:
            if self.WGF_NN:
                filename = "data/data_nSamples_" + str(self.nproc*self.particle.number_particles_all) \
                        + "_isProjection_" + str(self.options["is_projection"]) + "_WGF_NN.p"
            elif self.WGF_cvxNN:
                filename = "data/data_nSamples_" + str(self.nproc*self.particle.number_particles_all) \
                        + "_isProjection_" + str(self.options["is_projection"]) + "_WGF_cvxNN.p"
            else:
                filename = "data/data_nSamples_" + str(self.nproc*self.particle.number_particles_all) \
                        + "_isProjection_" + str(self.options["is_projection"]) + "_WGF.p"
        else:
            filename = "data/data_nSamples_" + str(self.nproc*self.particle.number_particles_all) \
    ...
    def gradientSeparated_WGF(self, gradient, m):
        ...
        gp_misfit = np.zeros(gradient.shape)
    ...
    def get_grad(self):
        ...
        for m in range(self.particle.number_particles):
            grad[m,:] = self.variation.gradient_gather[self.rank][m].copy()
        return grad
