...
class Variation:
    ...
    def update(self, particle, iter=0, step_norm=1, batch=False):
        ...
        if self.options["is_projection"]:
            ...
            self.is_bases_updated = False
            if ((np.mod(iter, self.options["update_step"]) == 0) or (step_norm < self.options["step_projection_tolerance"])) and not batch:  # np.mod(iter, particle.coefficient_dimension)
